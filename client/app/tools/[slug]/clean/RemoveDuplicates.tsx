...
export default function RemoveDuplicates() {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setPreviewHeaders([]);
          setPreviewRows([]);
          setPreviewError(e instanceof Error ? e.message : "Could not load preview rows");
        }
      } finally {
        if (!cancelled) {
  ...
  return (
    <div className="space-y-4">
      <FileUploadDropzone
        accept={EXCEL_ACCEPT}
        message="Drop or select an Excel file to remove duplicate rows"
        hasError={!!error}
        onFiles={(files) => {
      ...
      {preview ? (
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <h3 className="font-medium">Target rows</h3>
              <p className="text-sm" style={{ color: "var(--muted-2)" }}>
                {fileName} - {preview.sheet_count} sheet{preview.sheet_count === 1 ? "" : "s"}
              </p>
            </div>
            <span
                      ...
                      {previewHeaderCells.map((header, index) => {
                        ...
                        return (
                          <th
                            ...
                            title={
                              !isSelectable
                                ? "Unnamed header cannot be selected"
                                : "Click to toggle this column"
                            }
                          >
          ...
          {currentStep === 3 ? (
            <div className="space-y-4">
              ...
              >
                ...
                <div>
                  Duplicates: <strong>{countingDuplicates ? "Counting..." : (duplicateCount?.toLocaleString() ?? "0")}</strong>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="tool-primary-action inline-flex cursor-pointer items-center gap-2 rounded-md px-4 py-2 text-sm font-medium disabled:opacity-50"
                >
                  {loading ? "Processing..." : "Download Clean Workbook"}
                </button>
              </div>
            </div>
          ) : null}
