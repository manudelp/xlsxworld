...
export default function AppendWorkbooks() {
  ...
  const onFiles = useCallback((selectedFiles: FileList | null) => {
    ...
    if (newUniqueFiles.length === 0) {
      setOrderFeedback(t("noNewFilesAdded"));
      if (files.length < 2) {
        setError(t("addAtLeastTwo"));
      }
      return;
    }
    ...
    setOrderFeedback(
      t("addedFiles", { count: newUniqueFiles.length, total: nextFiles.length }),
    );
  ...
  const handleAppend = useCallback(async () => {
    if (files.length < 2) {
      setError(t("selectAtLeastTwoFiles"));
      return;
    }
        ...
        >
            ...
            >
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 text-xs md:grid-cols-4">
                ...
                >
                  <div>{t("orderMode")}</div>
                  <div className="font-medium" style={{ color: "var(--foreground)" }}>
                    {isUploadOrder ? t("uploadOrder") : t("customOrder")}
                  </div>
                </div>
                <div
                  className="rounded border px-2 py-1"
          ...
          >
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <div className="text-sm font-medium">{t("appendOrder")}</div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  aria-label={isOrderExpanded ? t("collapseAppendOrder") : t("expandAppendOrder")}
                >
                  <span className="inline-flex items-center gap-1.5 leading-none">
                    {isOrderExpanded ? t("collapse") : t("expand")}
                    {isOrderExpanded ? (
