...
export default function ConvertXlsxToXml() {
  const t = useTranslations("common");
  const [preview, setPreview] = useState<WorkbookPreview | null>(null);
  const [file, setFile] = useState<File | null>(null);
  const [activeSheet, setActiveSheet] = useState(0);
  const [selectedSheets, setSelectedSheets] = useState<string[]>([]);
  const [rootTag, setRootTag] = useState("workbook");
  ...
  const download = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const buf = await xlsxToXml(file, selectedSheets, rootTag, rowTag);
      const blob = new Blob([buf], { type: "application/xml" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      const baseName = file.name.replace(/\.[^.]+$/, "");
      a.download = `${baseName || "workbook"}.xml`;
  ...
  }, [file, selectedSheets, rootTag, rowTag, t]);

  return (
    <div className="space-y-4">
      <FileUploadDropzone
        accept={EXCEL_ACCEPT}
        message={t("dropExcelConvertXml")}
        hasError={!!error}
        onFiles={(files) => {
          const f = files[0];
          if (f) void onFile(f);
      ...
      {preview && (
        ...
        >
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <div>
              <h3 className="font-medium">{t("selectSheetsToExportXml")}</h3>
      ...
      {preview && (
        <div className="flex justify-end">
          ...
          >
            ...
            {loading ? t("exporting") : t("downloadXml")}
          </button>
        </div>
      )}
    </div>
