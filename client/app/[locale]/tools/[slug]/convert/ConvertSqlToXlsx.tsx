...
import { sqlToXlsx } from "@/lib/tools/convert";
import FileUploadDropzone from "@/components/common/FileUploadDropzone";

export default function ConvertSqlToXlsx() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const buf = await sqlToXlsx(file, includeHeaders);
  ...
  return (
    <div className="space-y-4">
      <FileUploadDropzone
        accept=".sql,application/sql,text/sql,text/x-sql,text/plain"
        message={t("dropSqlConvertXlsx")}
        hasError={!!error}
        onFiles={(files) => {
      ...
      {file && (
        ...
        >
          <h3 className="mb-2 font-medium">{t("sqlToXlsxInfo")}</h3>
          <ul className="space-y-1 text-sm" style={{ color: "var(--muted)" }}>
            <li>{t("sqlInsertStatementsParsed")}</li>
            <li>{t("sqlOneSheetPerTable")}</li>
          </ul>
