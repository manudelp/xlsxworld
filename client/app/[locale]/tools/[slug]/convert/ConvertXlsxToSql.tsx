...
export default function ConvertXlsxToSql() {
  ...
  const download = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const buf = await xlsxToSql(file, selectedSheets, tablePrefix);
      const blob = new Blob([buf], { type: "application/sql" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      const baseName = file.name.replace(/\.[^.]+$/, "");
      a.download = `${baseName || "workbook"}.sql`;
  ...
  }, [file, selectedSheets, tablePrefix, t]);
    ...
    <div className="space-y-4">
      ...
      {preview && (
        ...
        >
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <div>
              <h3 className="font-medium">{t("selectSheetsToExportSql")}</h3>
      ...
      {preview && (
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          >
            {loading ? t("exporting") : t("downloadSql")}
          </button>
        </div>
      )}
    </div>
