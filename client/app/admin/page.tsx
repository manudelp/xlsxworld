...
type AdminPing = { pong: boolean };

export default function AdminPage() {
  const [data, setData] = useState<AdminPing | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const token = tokenStore.get();
    if (!token) { setError("Not logged in"); return; }
    const url = new URL("/api/auth/admin/ping", process.env.NEXT_PUBLIC_API_BASE || "http://localhost:8000");
